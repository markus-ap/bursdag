...
function datostreng(mån, dag, tim, min, sek){
    var res = "";
    if(mån != 0) res += (mån == 1) ? mån + " måned,<br>" : mån + " månedar,<br>"
    if(dag != 0) res += (dag == 1) ? dag + " dag,<br>" : dag + " dagar,<br>" 
    res += (tim == 1) ? tim + " time,<br>" : tim + " timar,<br>"

    res += min + " minutt,<br>og "+ sek + " sekund"
